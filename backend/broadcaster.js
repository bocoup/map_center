...
module.exports = function(httpServer, broadcastSchedule) {
        ...
        var bindings = {
            ...
            recording: function(broadcast) {
                ...
                var handleUpdateMap = function(mapState) {
                    ...
                    ioServer.sockets.emit("updateMap", mapState);
            ...
            replay: function(broadcast) {
                ...
                broadcastSchedule.getRecordingEvent(broadcast.recordingID, {}, function(err, recording) {
                    ...
                    _.forEach(recording, function(changeEvent) {
                        setTimeout(function() {
                            ioServer.sockets.emit("updateMap", changeEvent.mapState);
     ...
     *
     *     ioServer.sockets.on("updateMap", newUpdateMapHandler);
     *
    ...
    ioServer.of("/broadcaster").authorization(function(handshakeData, callback) {

        console.log("handshakeData", handshakeData);
        auth.checkSession(handshakeData.headers.cookie, function(err, isAuthorized) {
            if (err) {
                return callback(err);
            }
